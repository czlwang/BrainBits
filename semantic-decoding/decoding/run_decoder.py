...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_version", type = str, default = "orig")
    ...
    # load responses
    hf = h5py.File(os.path.join(config.DATA_TEST_DIR, "test_response", args.subject, args.experiment, args.data_version, args.task + ".hf5"), "r")
    resp = np.nan_to_num(hf["data"][:])
    hf.close()

    # load original responses
    print(os.path.join(config.DATA_TEST_DIR, "test_response", args.subject, args.experiment, "orig", args.task + ".hf5"))
    orig_hf = h5py.File(os.path.join(config.DATA_TEST_DIR, "test_response", args.subject, args.experiment, "orig", args.task + ".hf5"), "r")
    orig_resp = np.nan_to_num(orig_hf["data"][:])
    orig_hf.close()
    ...
    # load models
    load_location = os.path.join(config.MODEL_DIR, args.subject, args.data_version)
    ...
    # predict word times
    word_rate = predict_word_rate(orig_resp, word_rate_model["weights"], word_rate_model["voxels"], word_rate_model["mean_rate"])#CZW: note use the orig resp for rate prediction
    if args.experiment == "perceived_speech": word_times, tr_times = predict_word_times(word_rate, orig_resp, starttime = -10)
    else: word_times, tr_times = predict_word_times(word_rate, orig_resp, starttime = 0)
    lanczos_mat = get_lanczos_mat(word_times, tr_times)
    #word_rate2 = predict_word_rate(orig_resp, word_rate_model["weights"], word_rate_model["voxels"], word_rate_model["mean_rate"])#CZW: note use the orig resp for rate prediction
    ...
    save_location = os.path.join(config.RESULT_DIR, args.subject, args.experiment, args.data_version)
