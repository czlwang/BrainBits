...
parser.add_argument("-results_path", "--results_path",help="Results path",default=30)
parser.add_argument("-pred_latents_path", "--pred_latents_path",help="vdvae pred latents path",default=30)
args = parser.parse_args()
results_path = args.results_path
pred_latents_path = args.pred_latents_path
sub=int(args.sub)
...
image_path = '../brain-diffuser/data/processed_data/subj{:02d}/nsd_test_stim_sub{}.npy'.format(sub,sub)
...
for i in range(int(np.ceil(len(test_images)/batch_size))):
  ...
  for j in range(len(sample_from_latent)):
      ...
      im.save(os.path.join(results_path, f'{i*batch_size+j}.png'))
